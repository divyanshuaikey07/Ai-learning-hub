...
type Tool = {
  name: string; type: string; description: string; bestFor: string; access: string;
...
const tools: Tool[] = [
  { name: 'ChatGPT', type: 'Conversation', description: 'A flexible place to brainstorm, explain a topic, or rehearse an idea in plain language.', bestFor: 'Back-and-forth learning', access: 'Free tier available', tags: ['Writing', 'Explaining'], accent: 'teal' },
  { name: 'Claude', type: 'Conversation', description: 'A thoughtful chat workspace for working through long notes, drafts, and difficult questions.', bestFor: 'Reading & reflection', access: 'Free tier available', tags: ['Documents', 'Writing'], accent: 'coral' },
  { name: 'Perplexity', type: 'Research', description: 'A research-oriented search companion that helps you begin with a question and follow sources.', bestFor: 'Starting research', access: 'Free tier available', tags: ['Sources', 'Research'], accent: 'gold' },
  { name: 'NotebookLM', type: 'Study companion', description: 'A source-grounded notebook for asking questions about materials you provide.', bestFor: 'Class notes & packets', access: 'Free to try', tags: ['Notes', 'Study'], accent: 'teal' },
  { name: 'Elicit', type: 'Research', description: 'A guided research tool for finding and comparing academic papers around a question.', bestFor: 'Literature reviews', access: 'Free tier available', tags: ['Papers', 'Research'], accent: 'coral' },
  { name: 'Canva Magic Studio', type: 'Creative', description: 'A visual workspace with AI-assisted tools for slides, posters, and early concepts.', bestFor: 'Visual assignments', access: 'Free tier available', tags: ['Slides', 'Design'], accent: 'gold' },
  { name: 'Grammarly', type: 'Writing support', description: 'Writing feedback for clarity and tone. Keep your own voice in the final pass.', bestFor: 'Polishing a draft', access: 'Free tier available', tags: ['Editing', 'Writing'], accent: 'teal' },
  { name: 'Otter', type: 'Notes', description: 'A transcription and meeting-notes tool for turning spoken material into a reviewable outline.', bestFor: 'Reviewing lectures', access: 'Free tier available', tags: ['Transcripts', 'Notes'], accent: 'coral' },
];
...
function ToolsPage() {
  ...
  const filtered = useMemo(() => tools.filter((tool) => `${tool.name} ${tool.description} ${tool.tags.join(' ')}`.toLowerCase().includes(query.toLowerCase()) && (type === 'All types' || tool.type === type)), [query, type]);
  const types = ['All types', ...Array.from(new Set(tools.map((tool) => tool.type)))];
  return <><Meta title="AI Tools Directory — StudyPilot" description="A thoughtful directory of AI tools with plain-language descriptions and practical use cases." /><PageIntro eyebrow="The field kit" title="Tools worth knowing about." text="A considered directory of AI tools for learning, writing, research, and making. Start with the job you have, not the tool everyone is talking about." icon={Zap} /><div className="mx-auto max-w-[1240px] px-5 py-12 lg:px-8 lg:py-16"><div className="flex flex-col gap-3 rounded-2xl border border-border bg-card p-3 shadow-paper md:flex-row"><div className="flex flex-1 items-center gap-3 rounded-xl bg-secondary/60 px-4"><Search size={18} className="text-muted-foreground" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search tools, tasks, or tags" className="w-full bg-transparent py-3 text-sm outline-none" aria-label="Search tools" data-testid="input-tools-search" /></div><div className="flex items-center gap-2 overflow-x-auto px-1"><SlidersHorizontal size={16} className="shrink-0 text-muted-foreground" />{types.map((item) => <button key={item} onClick={() => setType(item)} className={`whitespace-nowrap rounded-full px-3 py-2 text-xs font-semibold ${type === item ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-secondary'}`} data-testid={`button-tool-filter-${item.toLowerCase().replaceAll(' ', '-')}`}>{item}</button>)}</div></div><p className="mt-8 font-mono text-[10px] uppercase tracking-[0.16em] text-muted-foreground" data-testid="text-tools-count">{filtered.length} tools in the field kit</p>{filtered.length ? <div className="mt-4 grid gap-4 md:grid-cols-2">{filtered.map((tool) => <ToolCard key={tool.name} tool={tool} />)}</div> : <EmptySearch label="No tools match that search." onReset={() => { setQuery(''); setType('All types'); }} />}</div></>;
}
...
function ToolCard({ tool }: { tool: Tool }) {
  ...
  return <article className="group rounded-2xl border border-border bg-card p-6 transition-all hover:-translate-y-1 hover:border-primary/30 hover:shadow-paper" data-testid={`card-tool-${tool.name.toLowerCase().replaceAll(' ', '-')}`}><div className="flex items-start justify-between gap-4"><div className={`flex h-11 w-11 items-center justify-center rounded-2xl text-lg font-bold ${tone}`}>{tool.name.charAt(0)}</div><span className="rounded-full bg-secondary px-3 py-1 font-mono text-[10px] uppercase tracking-[0.12em] text-muted-foreground">{tool.type}</span></div><h2 className="mt-6 font-serif text-2xl tracking-[-0.025em]">{tool.name}</h2><p className="mt-2 min-h-[48px] text-sm leading-6 text-muted-foreground">{tool.description}</p><div className="mt-6 border-t border-border pt-4"><div className="flex justify-between text-xs"><span className="text-muted-foreground">Best for</span><strong>{tool.bestFor}</strong></div><div className="mt-3 flex items-center justify-between"><span className="text-xs text-muted-foreground">{tool.access}</span><div className="flex gap-1.5">{tool.tags.map((tag) => <span key={tag} className="rounded-full border border-border px-2 py-1 text-[10px] text-muted-foreground">{tag}</span>)}</div></div></div></article>;
}
