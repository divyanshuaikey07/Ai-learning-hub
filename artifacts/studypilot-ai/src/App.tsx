...
type Tool = {
  name: string; type: string; description: string; bestFor: string; access: string; url: string;
...
const tools: Tool[] = [
  { name: 'ChatGPT', type: 'AI Chatbots', description: 'A flexible place to brainstorm, explain a topic, or rehearse an idea in plain language.', bestFor: 'Back-and-forth learning', access: 'Official site', url: 'https://chatgpt.com/', tags: ['Writing', 'Explaining'], accent: 'teal' },
  { name: 'Claude', type: 'AI Chatbots', description: 'A thoughtful chat workspace for working through long notes, drafts, and difficult questions.', bestFor: 'Reading & reflection', access: 'Official site', url: 'https://claude.ai/', tags: ['Documents', 'Writing'], accent: 'coral' },
  { name: 'Gemini', type: 'AI Chatbots', description: 'Google’s conversational assistant for exploring ideas, getting explanations, and working across formats.', bestFor: 'Everyday questions', access: 'Official site', url: 'https://gemini.google.com/', tags: ['Explaining', 'Ideas'], accent: 'gold' },
  { name: 'Midjourney', type: 'Image Generation', description: 'A creative image-making tool for exploring visual directions from a written prompt.', bestFor: 'Visual concepts', access: 'Official site', url: 'https://www.midjourney.com/', tags: ['Images', 'Concepts'], accent: 'coral' },
  { name: 'Adobe Firefly', type: 'Image Generation', description: 'Adobe’s generative tools for creating and exploring images, styles, and visual variations.', bestFor: 'Design exploration', access: 'Official site', url: 'https://firefly.adobe.com/', tags: ['Images', 'Design'], accent: 'gold' },
  { name: 'Runway', type: 'Video', description: 'A creative toolkit for experimenting with AI-assisted video, motion, and visual storytelling.', bestFor: 'Short video ideas', access: 'Official site', url: 'https://runwayml.com/', tags: ['Video', 'Motion'], accent: 'teal' },
  { name: 'Descript', type: 'Video', description: 'An editor that makes working with video and audio feel closer to editing a document.', bestFor: 'Editing & captions', access: 'Official site', url: 'https://www.descript.com/', tags: ['Editing', 'Audio'], accent: 'coral' },
  { name: 'Grammarly', type: 'Writing', description: 'Writing feedback for clarity and tone. Keep your own voice in the final pass.', bestFor: 'Polishing a draft', access: 'Official site', url: 'https://www.grammarly.com/', tags: ['Editing', 'Writing'], accent: 'teal' },
  { name: 'QuillBot', type: 'Writing', description: 'A writing assistant for rephrasing, summarizing, and finding a clearer way to express an idea.', bestFor: 'Reworking a draft', access: 'Official site', url: 'https://quillbot.com/', tags: ['Rewriting', 'Summaries'], accent: 'gold' },
  { name: 'GitHub Copilot', type: 'Coding', description: 'A coding companion that can explain code, suggest next steps, and help you learn by example.', bestFor: 'Learning to code', access: 'Official site', url: 'https://github.com/features/copilot', tags: ['Code', 'Explaining'], accent: 'coral' },
  { name: 'Cursor', type: 'Coding', description: 'An AI-first code editor for asking questions about a project and moving from idea to implementation.', bestFor: 'Project work', access: 'Official site', url: 'https://www.cursor.com/', tags: ['Editor', 'Code'], accent: 'teal' },
  { name: 'Notion AI', type: 'Productivity', description: 'An assistant inside a flexible workspace for organizing notes, plans, and project material.', bestFor: 'Planning & notes', access: 'Official site', url: 'https://www.notion.so/product/ai', tags: ['Notes', 'Planning'], accent: 'gold' },
  { name: 'Otter', type: 'Productivity', description: 'A transcription and meeting-notes tool for turning spoken material into a reviewable outline.', bestFor: 'Reviewing lectures', access: 'Official site', url: 'https://otter.ai/', tags: ['Transcripts', 'Notes'], accent: 'coral' },
  { name: 'NotebookLM', type: 'Education', description: 'A source-grounded notebook for asking questions about materials you provide.', bestFor: 'Class notes & packets', access: 'Official site', url: 'https://notebooklm.google.com/', tags: ['Notes', 'Study'], accent: 'teal' },
  { name: 'Elicit', type: 'Education', description: 'A guided research tool for finding and comparing academic papers around a question.', bestFor: 'Literature reviews', access: 'Official site', url: 'https://elicit.com/', tags: ['Papers', 'Research'], accent: 'coral' },
  { name: 'Canva Magic Studio', type: 'Education', description: 'A visual workspace with AI-assisted tools for slides, posters, and early concepts.', bestFor: 'Visual assignments', access: 'Official site', url: 'https://www.canva.com/magic-studio/', tags: ['Slides', 'Design'], accent: 'gold' },
];
...
function ToolCard({ tool, index }: { tool: Tool; index: number }) {
  ...
  const CategoryIcon = tool.type === 'AI Chatbots' ? BrainCircuit : tool.type === 'Image Generation' ? Sparkles : tool.type === 'Video' ? Film : tool.type === 'Writing' ? BookOpen : tool.type === 'Coding' ? Zap : tool.type === 'Productivity' ? Layers3 : ShieldCheck;
  return <article className={`group card-reveal flex h-full flex-col rounded-2xl border border-border bg-card p-5 transition-all duration-300 hover:-translate-y-1 hover:border-primary/30 hover:shadow-paper [animation-delay:${Math.min(index, 8) * 45}ms]`} data-testid={`card-tool-${tool.name.toLowerCase().replaceAll(' ', '-')}`}><div className="flex items-start justify-between gap-4"><div className={`flex h-11 w-11 items-center justify-center rounded-2xl ${tone} transition-transform duration-300 group-hover:scale-105`}><CategoryIcon size={20} strokeWidth={1.6} /></div><span className="rounded-full bg-secondary px-3 py-1 font-mono text-[10px] uppercase tracking-[0.12em] text-muted-foreground">{tool.type}</span></div><h2 className="mt-5 font-serif text-2xl tracking-[-0.025em]">{tool.name}</h2><p className="mt-2 min-h-[72px] text-sm leading-6 text-muted-foreground">{tool.description}</p><div className="mt-5 border-t border-border pt-4"><div className="flex justify-between gap-3 text-xs"><span className="text-muted-foreground">Best for</span><strong className="text-right">{tool.bestFor}</strong></div><div className="mt-4 flex flex-wrap items-center justify-between gap-2"><span className="text-xs text-muted-foreground">{tool.access}</span><div className="flex flex-wrap justify-end gap-1.5">{tool.tags.map((tag) => <span key={tag} className="rounded-full border border-border px-2 py-1 text-[10px] text-muted-foreground">{tag}</span>)}</div></div></div><a href={tool.url} target="_blank" rel="noopener noreferrer" className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-semibold text-primary-foreground transition-all hover:-translate-y-0.5 hover:brightness-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2" data-testid={`link-tool-try-${tool.name.toLowerCase().replaceAll(' ', '-')}`}>Try tool <ArrowUpRight size={15} /></a></article>;
}
